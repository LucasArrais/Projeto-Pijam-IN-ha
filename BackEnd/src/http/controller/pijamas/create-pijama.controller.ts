...
export async function createPijama(
  ...
  try {
    ...
    const {
      ...
      on_sale,
      sale_percent,
    } = createPijamaBodySchema.parse(request.body)

    const createPijamaUseCase = makeCreatePijamaUseCase()

    const { pijama } = await createPijamaUseCase.execute({
      name,
      description,
      image,
      price,
    ...
    return reply.status(201).send(PijamaPresenter.toHTTP(pijama))
  } catch (error) {
